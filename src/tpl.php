...
class tpl
{
    ...
    function __construct()
    {
        ...
        $timemod = function ($data, $mod_ext, $spaces, $key, $mod) {
            ...
            return $spaces . self::toRusDate($data, $format);
        };
...
//        'may'=>'мая', - уже есть
    ...
    private function _($sql, $param, $type = 'text', $quote = null)
    {
        ...
        $repl = function ($eq, $spaces, $data, $key, $quote, $mod = '') use (&$last) {
            ...
            if (is_array($data) || is_object($data)) {
                ...
                if (is_numeric($data)) $last = $data;
                if (!empty($mod)) {
                    $x = explode('|', $mod);
                    if (count($x) == 4) { // pluralform
                        return $eq . $key . $this->plural((int)$last, $x[1], $x[2], $x[3]);
                    }
                    $mod = trim($x[1]);
                    array_shift($x);
                    array_shift($x);
    ...
    public static function __callStatic($method, array $parameters)
    {
        ...
        if ($method == '_') {
            if(is_subclass_of($parameters[0],__CLASS__)){
                $me=$parameters[0]; return;
            }
