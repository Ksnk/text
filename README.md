...
В шаблонах используется более компактная форма вызова - одна строка параметров, 
разделенных `|`.
...
Чудеса эскейпинга можно продемонстрировать на примере. Технически, не обязательно эскейпить все 
...
нужные слеши в PHP будет `addcslashes($x,'{}?:\|');`
    ...
    // все служебные символы прослешены, но синтаксис позволяет
    $pattern = 'Сумма прописью:{ Очень странная переменная \{1\|2\}\?\: }';
...
`{first_name?{first_name}:посетитель}`
    ...
    echo tpl::text('{number} баран{number||а|ов} на поле!', $data);
    echo tpl::text('{number} {баран||а|ов} на поле!', $data);
    // 23 барана на поле
...
Окончание числительных. Модификатор, в котором присутствует 3 символа `|`.
 ...
 Вариант `{баран||а|ов}` фактически совпадает с форматом параметра plural функции, однако при его использовании следует 
